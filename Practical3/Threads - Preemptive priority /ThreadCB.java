...
{
	private static GenericList readyQueue1;
	private static GenericList readyQueue2;
	private static GenericList readyQueue3;
	...
	{
		readyQueue1 = new GenericList();
		readyQueue2 = new GenericList();
		readyQueue3 = new GenericList();
	}
	...
	static public ThreadCB do_create(TaskCB task)
	{
		...
		if (task.getThreadCount() >= MaxThreadsPerTask)
		{
			MyOut.print("osp.Threads.ThreadCB",
					"Failed to create new thread " + " -- maximum number of threads for " + task + " reached");
			dispatch();
			return null;
		}
		...
		newThread.setPriority(random.nextInt(3));
	...
	public static int do_dispatch()
	{
		...
		if (runningThread != null)
		{
			...
			MyOut.print("osp.Threads.ThreadCB", "Removing thread with priority: " + runningThread.getPriority());
		...
		{
			if(readyQueue2.length() <= 0)
				threadToDispatch = (ThreadCB) readyQueue3.removeHead();
			else
				threadToDispatch = (ThreadCB) readyQueue2.removeHead();
		}
		else
			threadToDispatch = (ThreadCB) readyQueue1.removeHead();
		...
		MyOut.print("osp.Threads.ThreadCB", "Replacing with a thread of priority: " + threadToDispatch.getPriority());
